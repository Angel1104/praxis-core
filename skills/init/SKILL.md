...
name: init
description: >
  Configures the SDM skills pack for a brand new project with no code yet.
...
If any exist:
> "This directory already has code. Use `/setup --[platform]` to configure an
> existing project instead."

Stop.

If `CLAUDE.md` exists with `SDM Platform:`:
> "This project is already configured. Use `/setup --[platform]` to regenerate
...
- After a project initialised with `/init` grows significantly, re-run `/setup --[platform]`
