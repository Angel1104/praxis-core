...
2. Load project context — in this order, stop as soon as you have enough:
   - Read `CLAUDE.md` — check `SDM Domain:` and `SDM Gates:` lines
...
4. Determine the rigor level and derive the pipeline from type + rigor:
   ...
   |------|--------------|----------|
   | `feature` | standard | spec (full) → plan → build → review → close |
   | `fix` | standard | spec (lean) → build → review → close |
   | `refactor` | standard | spec (lean) → build → review → close |
   | `security` | standard | spec (full) → plan → build → review → close |
   | `incident` | fast | build (containment first) → review → close |
...
> Pipeline: [full sequence, e.g. "spec (lean) → build → review → close"]
>
